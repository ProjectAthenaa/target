...
func (tk *Task) APIKey() {
	...
	if err != nil {
		tk.SetStatus(module.STATUS_ERROR, err, "could not make homepage req")
		tk.Stop()
		return
	}
	...
	res, err := tk.Do(req)
	if err != nil {
		tk.SetStatus(module.STATUS_ERROR, err, "could not fetch site")
		tk.Stop()
		return
	}
...
func (tk *Task) OauthPost() {
	...
	if err != nil {
		tk.SetStatus(module.STATUS_ERROR, err, "could not create oauth request")
		tk.Stop()
		return
	}
	...
	_, err = tk.Do(req)
	if err != nil {
		tk.SetStatus(module.STATUS_ERROR, err, "could not fetch oauth request")
		tk.Stop()
		return
	}
}
...
func (tk *Task) OauthSession() {
	...
	if err != nil {
		tk.SetStatus(module.STATUS_ERROR, err, "could not create oauth2 second request")
		tk.Stop()
		return
	}
	...
	res, err := tk.Do(req)
	if err != nil {
		tk.SetStatus(module.STATUS_ERROR, err, "could not get oauth2 second request")
		tk.Stop()
		return
	}
...
func (tk *Task) GetResString() *string {
	...
	if err != nil {
		tk.SetStatus(module.STATUS_ERROR, err, "couldnt create get req to base script for ponos")
		tk.Stop()
		return nil
	}
	...
	if err != nil {
		tk.SetStatus(module.STATUS_ERROR, err, "couldnt get base script for ponos")
		tk.Stop()
		return nil
	}
	...
	if err != nil {
		tk.SetStatus(module.STATUS_ERROR, err)
		tk.Stop()
		return nil
	}
...
func (tk *Task) Login() {
	...
	if err != nil {
		tk.SetStatus(module.STATUS_ERROR, err, "error creating login request")
		tk.Stop()
		return
	}
	...
	tk.SetStatus(module.STATUS_GENERATING_COOKIES, err)

	headers, err := shapeClient.GenHeaders(tk.Ctx, &shape.Site{Value: shape.SITE_TARGET, ResString: tk.GetResString()})
	if err != nil {
		tk.SetStatus(module.STATUS_ERROR, err, "error generating shape headers")
	}
	...
	tk.SetStatus(module.STATUS_LOGGING_IN, err)

	res, err := tk.Do(req)
	if err != nil {
		tk.SetStatus(module.STATUS_ERROR, err, "error making login request")
		tk.Stop()
		return
	}
	...
	if e := loginErrRe.FindStringSubmatch(string(res.Body)); len(e) == 2 && tk.logincount < 3 {
		...
		tk.SetStatus(module.STATUS_ERROR, err, "login err "+e[1])
		tk.Stop()
		return
	}
	fmt.Println(string(res.Body))
	tk.guestid = guestIdRe.FindStringSubmatch(string(res.Body))[1]
	tk.SetStatus(module.STATUS_LOGGED_IN, err)
}
