...
func (tk *Task) RefreshToken() {
	...
	if err != nil {
		tk.SetStatus(module.STATUS_ERROR, err, "error creating refresh token request")
		tk.Stop()
		return
	}
	...
	_, err = tk.Do(req)
	if err != nil {
		tk.SetStatus(module.STATUS_ERROR, err, "error making refresh token request")
		tk.Stop()
		return
	}
}
...
func (tk *Task) OauthAuthCode() {
	...
	_, err = tk.Do(req)
	if err != nil {
		tk.SetStatus(module.STATUS_ERROR, err, "could not post auth code request")
		tk.Stop()
		return
	}
}
...
func (tk *Task) ClearCart() {
	...
	if err != nil {
		tk.SetStatus(module.STATUS_ERROR, err, "could not create clear cart request")
		tk.Stop()
		return
	}
	...
	res, err := tk.Do(req)
	if err != nil {
		tk.SetStatus(module.STATUS_ERROR, err, "could not post clear cart request")
		tk.Stop()
		return
	}

	if res.StatusCode == 403 {
		tk.SetStatus(module.STATUS_ERROR, err, "could not clear cart, likely proxy error")
		return
	}
	...
	for _, sm := range cartItemIdRe.FindAllSubmatch(res.Body, -1) {
		...
		if delres.StatusCode != 200 {
			tk.SetStatus(module.STATUS_ERROR, err, "could not delete item "+string(sm[1]))
		}
	}
}
